...
Первый запуск лучше начать с проверки — она создаёт слой, смотрит, что нашлось,
и сразу всё убирает, ничего не оставляя на экране:

```bash
./run.sh --overlay --probe         # слой + тач: работает или нет, и почему
./run.sh --overlay --timeout 60    # страховка: сам закроется через минуту
./run.sh --overlay --debug-touch   # крестик там, куда попало касание
```
...
| `--probe` | создать слой, проверить тач, отчитаться и выйти (ничего не оставляет на экране) |
| `--timeout N` | закрыться самому через N секунд |
