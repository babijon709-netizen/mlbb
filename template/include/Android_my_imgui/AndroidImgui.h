...
public:
    ...
    // Rebind the existing renderer/ImGui context to a newly created native
    // window. Used when OEM SurfaceFlinger cannot resize an input window in
    // place; textures and ImGui state remain alive.
    virtual bool ReplaceNativeWindow(ANativeWindow *window, float width, float height) = 0;
