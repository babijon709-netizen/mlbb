...
bool OpenGLGraphics::ReplaceNativeWindow(ANativeWindow *window, float width, float height) {
    if (!window || m_EglDisplay == EGL_NO_DISPLAY || m_EglContext == EGL_NO_CONTEXT)
        return false;
    ...
    EGLint num_configs = 0;
    if (!eglChooseConfig(m_EglDisplay, egl_attributes, nullptr, 0, &num_configs) ||
    ...
    EGLSurface new_surface = eglCreateWindowSurface(m_EglDisplay, egl_config, window, nullptr);
    if (new_surface == EGL_NO_SURFACE) return false;
    if (!eglMakeCurrent(m_EglDisplay, new_surface, new_surface, m_EglContext)) {
        eglDestroySurface(m_EglDisplay, new_surface);
        return false;
    }

    ANativeWindow *old_window = m_Window;
    EGLSurface old_surface = m_EglSurface;
    ANativeWindow_acquire(window);
    m_Window = window;
    m_Width = width;
    m_Height = height;
    m_EglSurface = new_surface;
    My_ImGui_ImplAndroid_Init(window);
    ImGuiIO &io = ImGui::GetIO();
    io.DisplaySize = ImVec2(width, height);

    if (old_surface != EGL_NO_SURFACE)
        eglDestroySurface(m_EglDisplay, old_surface);
    if (old_window)
